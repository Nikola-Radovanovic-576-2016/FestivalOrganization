...
namespace FestivalOrganization
{
    public partial class AddShow : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            myReader.Close();
            //validacija podataka vreme kraja nakon vremena pocetka, takodje trajanje predstave mora biti u okviru festivala
            if (DateTime.Compare(pocetak, kraj) < 0 && DateTime.Compare(pocetak, fest_pocetak) > 0 && DateTime.Compare(kraj, fest_kraj) < 0)
            {
                string query = @"insert into predstava(naziv_predstave, vreme_pocetka, vreme_zavrsetka, id_festivala, id_scene) VALUES " + "('" + naziv + "','" + pocetak + "','" + kraj + "','" + id_fest + "','" + id_bine + "')";
